    ...
    public:
...
#if 1	//dc42
                _pcb(pcb), _rx_buf(0), _rx_buf_offset(0), _discard_cb(discard_cb), _discard_cb_arg(discard_cb_arg), _refcnt(0), _next(0), _size_sent(0), _send_waiting(false) {
#else
        ...
        size_t write(const char* data, size_t size, bool last = true) {
			...
			while (size != 0)
			{
				...
				err_t err = tcp_write(_pcb, data, will_send, (last && will_send == size) ? TCP_WRITE_FLAG_COPY : TCP_WRITE_FLAG_COPY | TCP_WRITE_FLAG_MORE);
				if (err != ERR_OK)
				{
					DEBUGV(":wr !ERR_OK\r\n");
					return 0;
